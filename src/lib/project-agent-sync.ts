...
function requireScope(token: AgentTokenRecord, scope: ProjectAgentTokenScope) {
  if (!token.scopes.includes(scope)) throw new Error(`MISSING_SCOPE:${scope}`);
}
...
export async function applyAgentSync(token: AgentTokenRecord, payload: AgentSyncPayload, idempotencyKey: string) {
  requireValidPayload(payload);
  if (payload.projectId !== token.projectId) throw new Error("PROJECT_MISMATCH");
  ...
  const existing = await prisma.projectAgentSyncEvent.findUnique({ where: { tokenId_idempotencyKey: { tokenId: token.id, idempotencyKey } } });
  if (existing) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (payload.knowledge) {
      requireScope(token, "KNOWLEDGE_WRITE");
      const paths = payload.knowledge.map((item) => item.path);
      await tx.projectKnowledgeDocument.deleteMany({ where: { projectId: token.projectId, path: { notIn: paths } } });
      for (const item of payload.knowledge) {
        await tx.projectKnowledgeDocument.upsert({ where: { projectId_path: { projectId: token.projectId, path: item.path } }, create: { projectId: token.projectId, path: item.path, content: item.content, checksum: hash(item.content), sourceCommit: payload.sourceCommit ?? null }, update: { content: item.content, checksum: hash(item.content), sourceCommit: payload.sourceCommit ?? null, syncedAt: new Date() } });
        counts.knowledge++;
      }
    }
    if (payload.tasks?.length) {
      requireScope(token, "TASKS_WRITE");
      for (const item of payload.tasks) {
        const ref = externalId(item.id)!;
        await tx.devTask.upsert({
          where: { projectId_externalRef: { projectId: token.projectId, externalRef: ref } },
          create: { projectId: token.projectId, externalRef: ref, title: text(item.title, 180)!, description: item.description ? text(item.description, 4000) : null, status: taskStatus(item.status)!, priority: priority(item.priority) },
    ...
    if (payload.milestones?.length) {
      requireScope(token, "MILESTONES_WRITE");
      for (const item of payload.milestones) {
        const ref = externalId(item.id)!;
        await tx.milestone.upsert({
          where: { projectId_externalRef: { projectId: token.projectId, externalRef: ref } },
          create: { projectId: token.projectId, externalRef: ref, title: text(item.title, 180)!, dueDate: date(item.dueDate) ?? null, doneAt: item.done ? new Date() : null },
    ...
    if (payload.projectState) {
      requireScope(token, "PROJECT_STATE_WRITE");
      const state = payload.projectState;
      ...
      if (activity) await tx.activityLog.create({ data: { projectId: token.projectId, message: activity } });
      counts.projectState = true;
    }
    if (payload.update) {
      requireScope(token, "UPDATES_WRITE");
      await tx.projectUpdate.create({ data: { projectId: token.projectId, title: text(payload.update.title, 180)!, summary: text(payload.update.summary, 4000)!, kind: payload.update.kind ?? "INTERNAL", source: "AGENT", status: "DRAFT", nextSteps: payload.update.nextSteps?.map((item) => item.trim()).filter(Boolean).join("\n") || null, risks: payload.update.risks?.map((item) => item.trim()).filter(Boolean).join("\n") || null, createdByAgent: payload.agent } });
      counts.update = true;
    }
    const response = { ok: true, counts, projectId: token.projectId };
    await tx.projectAgentSyncEvent.create({ data: { projectId: token.projectId, tokenId: token.id, idempotencyKey, action: payload.action, payloadHash, resultJson: JSON.stringify(response) } });
    return response;
  });
