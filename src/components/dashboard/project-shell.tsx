...
export function ProjectShell({ currentProjectId, currentProjectName, projects, directConversations, assistantSessions, availableMembers, currentUser, children }: ProjectShellProps) {
  ...
  return <div className="h-screen overflow-hidden bg-[var(--background)] text-[var(--foreground)]"><div className="grid h-screen grid-cols-[220px_minmax(0,1fr)]">
    <aside className="senda-project-sidebar flex h-screen min-h-0 flex-col overflow-hidden border-r border-[var(--border-soft)] bg-[var(--surface-muted)] px-3 py-4">
      ...
      <div className="relative mt-6 shrink-0 px-1">
        <button type="button" onClick={() => setShowProjectMenu((open) => !open)} className="flex w-full items-center gap-2 rounded-xl border border-[var(--border-soft)] bg-[var(--surface)] px-3 py-2.5 text-left shadow-sm transition hover:border-teal-600/30" aria-expanded={showProjectMenu}><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-teal-50 text-xs font-bold text-teal-700">{currentProjectName.slice(0, 1).toUpperCase()}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold">{currentProjectName}</span><span className="mt-0.5 block text-[10px] text-zinc-500">Proyecto activo</span></span><span className="text-xs text-zinc-400">⌄</span></button>
        {showProjectMenu ? <div className="absolute left-1 right-1 top-[calc(100%+8px)] z-30 max-h-72 overflow-y-auto rounded-xl border border-[var(--border-soft)] bg-[var(--surface)] p-2 shadow-xl"><p className="px-2 pb-1 text-[10px] font-bold uppercase tracking-[.14em] text-zinc-400">Tus proyectos</p>{projects.map((project) => <Link key={project.id} href={`/projects/${project.id}`} onClick={() => setShowProjectMenu(false)} className={`block rounded-lg px-2.5 py-2 ${project.id === currentProjectId ? "bg-[var(--surface-strong)]" : "hover:bg-[var(--surface-strong)]"}`}><span className="flex items-center justify-between gap-2"><span className="truncate text-xs font-semibold">{project.name}</span><span className="shrink-0 text-[10px] text-teal-600">{project.progress}%</span></span><span className="mt-0.5 block truncate text-[10px] text-zinc-500">{project.phase}</span></Link>)}</div> : null}
      </div>
